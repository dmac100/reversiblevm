...
public class BrushLua extends Brush {
  public static final Pattern multiLineSingleQuotedString = Pattern.compile("'((\\\\.)|([^\\\\']))*+'", Pattern.DOTALL);	
	
  public BrushLua() {
    super();

	String keywords = "and break do else elseif end false for function if in local nil not or repeat "
		+ "return then true until while";

	String funcs = "_VERSION assert collectgarbage dofile error gcinfo loadfile loadstring print "
    ...
    _regExpRuleList.add(new RegExpRule(RegExpRule.multiLineDoubleQuotedString, "string"));
    ...
    _regExpRuleList.add(new RegExpRule(getKeywords(funcs), Pattern.MULTILINE, "functions"));
    _regExpRuleList.add(new RegExpRule("0x[a-f0-9]+|\\d+(\\.\\d+)?", Pattern.CASE_INSENSITIVE, "value"));
    setRegExpRuleList(_regExpRuleList);

    setCommonFileExtensionList(Arrays.asList("lua", "lua"));
  }
}
